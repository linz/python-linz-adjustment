...
def _mergedSets(haobs):
    ...
    while hasets:
        trialset = hasets.pop()
        mergeset = next(
            (ha for ha in hasets if not ha.targets.isdisjoint(trialset.targets)), None
        )
...
class MergeAnglesPlugin(Plugin):
    ...
    def preSetup(self):
        self.adjustment.observations = _mergeAngleObservations(
            self.adjustment.observations
        )
