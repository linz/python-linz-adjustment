...
class Station(object):
    ...
    def setXYZ(self, xyz):
        ...
        self._genu = Station._ellipsoid.enu_axes(
            lon + self._xieta[1] / math.cos(math.radians(lat)), lat + self._xieta[0]
        )
    ...
    def vectorTo(
        ...
        diff = trgtstn.xyz(trgtofst, offsettype=offsettype) - self.xyz(
            instofst, offsettype=offsettype
        )
        if not ddxyz:
            return diff
    ...
    def zenithDistanceTo(
        ...
        # (wrt self coordinates)
        diff = np.array(
            [denu[2] * denu[0] / hordist, denu[2] * denu[1] / hordist, -hordist]
        )
        diff /= np.vdot(diff, diff)
        # Convert to XYZ
        ...
        if instofst is not None or trgtofst is not None:
            ...
            if offsettype == Station.OFFSET_H:
                ...
                if offsettype == Station.OFFSET_XYZ:
                    ...
                    raise RuntimeError(
                        "Invalid offsettype in Station.heightDifferentTo"
                    )
