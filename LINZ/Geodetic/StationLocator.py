...
class StationLocator(object):
    ...
    class AngleSet(object):
        ...
        def resetStatus(self, status=None):
            oldstatus = {"orientation": self.orientation, "defined": self.defined}
            status = status or {}
    ...
    class Station(object):
        ...
        def addObs(self, tostn, obstype, obsvalue, obsset=None, reverse=False):
            ...
            if obstype not in toobs:
                toobs[obstype] = []
            toobs[obstype].append(
                StationLocator.ObsDef(tostn, obstype, obsvalue, obsset, reverse)
            )
        ...
        def estimatedLocation(self):
            ...
            for trialxyz in list(self.trialXyz.values()):
                xyz += trialxyz
            xyz /= ncoords
            return xyz
        ...
        def fixStation(self, xyz, exclude=None):
            ...
            for tostn in self.obs:
                ...
                az0 = azimuths[0]
                azimuths = (
                    np.remainder(np.array(azimuths) - az0 + math.pi, 2 * math.pi)
                    + az0
                    - math.pi
                )
                ...
                denu = np.array(
                    (hordist * math.sin(azimuth), hordist * math.cos(azimuth), hgtdiff)
                )
        ...
        if len(self.unlocated) > 0:
            while self.tryLocateStation() or self.tryFixAngle():
                ...
                if len(self.unlocated) >= nunlocated:
                    raise RuntimeError(
                        "tryLocateStation or tryFixAngle failed to fix stations"
                    )
                nunlocated = len(self.unlocated)
                if nunlocated == 0:
                    break
    ...
    def resetUnlocatedStations(self):
        self.unlocated = [s for s in list(self.stations.values()) if s.xyz is None]
    ...
    def tryLocateStation(self, exclude=None):
        ...
        self.write(
            "Determined {0} from {1} trial coords as {2}\n".format(
                maxtrialstn.code, maxtrialcoords, str(xyz)
            )
        )
    ...
    def resetStatus(self, status=None):
        ...
        return {"stations": stnold, "sets": setold}
    ...
    def tryFixAngle(self):
        ...
        try:
            while len(unfixedAngles) > 0:
                ...
                if trialxyz is None:
                    write(
                        "  Cannot determine pivoted coordinate for {0}\n".format(
                            trialstation.code
                        )
                    )
                ...
                write(
                    "Fixed angle connects {0} stations - {1} have known coordinates\n".format(
                        len(newfixed), len(commonStations)
                    )
                )
                ...
                for s in newfixed:
                    pivot = (
                        ...
                        if s in commonStations
                        else "new"
                    )
                    llh = GRS80.geodetic(s.xyz)
                    write(
                        'stnloccsv,"{0}",{1},{2},{3:.8f},{4:.8f},{5:.4f}\n'.format(
            ...
            if not success:
                if savedStatus is not None:
                    self.resetStatus(savedStatus)
                    write(
                        "Restored {0} original fixed stations\n".format(
                            len(fixedStations)
                        )
                    )
        finally:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate approximate station coordinates"
    )
    ...
    parser.add_argument(
        ...
        "--no-input-coords",
        action="store_true",
        help="Input coordinate file does not exist",
    )
    parser.add_argument(
        "-g",
        "--geodetic",
        action="store_true",
        help="Output geodetic coordinate (ie as lon lat hgt)",
    )
